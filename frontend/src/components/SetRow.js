...
export default function SetRow({ setNumber, values, onChange, onRemove, canRemove }) {
  return (
    <div className="flex items-center gap-2">
      {/* Set number badge */}
      <span className="w-7 text-xs text-dark-muted text-center font-semibold flex-shrink-0">{setNumber}</span>
      ...
      <button
        onClick={onRemove}
        disabled={!canRemove}
        className="icon-btn text-dark-subtle hover:text-red-500 transition-colors disabled:opacity-0 flex-shrink-0"
      >
